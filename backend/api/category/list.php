...
$sortBy = isset($_GET['sort_by']) ? trim((string) $_GET['sort_by']) : 'sort_order';
...
try {
    ...
    if ($q !== '') {
        $where[] = "(
            ...
            OR name LIKE :q_name
            OR slug LIKE :q_slug
            OR description LIKE :q_description
        ...
        $params = [
            ...
            ':q_name' => $searchValue,
            ':q_slug' => $searchValue,
            ':q_description' => $searchValue,
            ':q_sort_order' => $searchValue,
            ':q_created_at' => $searchValue,
            ':q_updated_at' => $searchValue
        ];
    }
    ...
    $formattedCategories = array_map(static function (array $category): array {
        return [
            'id' => (int) $category['id'],
            'name' => $category['name'],
            'slug' => $category['slug'],
