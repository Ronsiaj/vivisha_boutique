...
$wishlistIdInput = isset($_GET['wishlist_id']) ? trim((string)$_GET['wishlist_id']) : '';
...
$wishlistId = wishlistPositiveId($wishlistIdInput, 'wishlist_id');
$productId = wishlistPositiveId($productIdInput, 'product_id');
$variantId = wishlistPositiveId($variantIdInput, 'variant_id');
$categoryId = wishlistPositiveId($categoryIdInput, 'category_id');
$sizeId = wishlistPositiveId($sizeIdInput, 'size_id');
$colorId = wishlistPositiveId($colorIdInput, 'color_id');

$isAvailableInput = isset($_GET['is_available'])
    ? trim((string)$_GET['is_available'])
    : '';

$isAvailable = wishlistBoolean(
    $isAvailableInput,
    'is_available'
);
...
$isNewArrival = wishlistBoolean(
    $isNewArrivalInput,
    'is_new_arrival'
);

$isFeatured = wishlistBoolean(
    $isFeaturedInput,
    'is_featured'
);

$isBestSeller = wishlistBoolean(
    $isBestSellerInput,
    'is_best_seller'
);
...
$minPrice = wishlistPrice($minPriceInput, 'min_price');
$maxPrice = wishlistPrice($maxPriceInput, 'max_price');
$minOriginalPrice = wishlistPrice($minOriginalPriceInput, 'min_original_price');
$maxOriginalPrice = wishlistPrice($maxOriginalPriceInput, 'max_original_price');
...
try {
    ...
    if ($q !== '') {
        $where[] = "(
            CAST(w.id AS CHAR) LIKE :q_wishlist_id
            OR CAST(w.product_id AS CHAR) LIKE :q_product_id
            OR CAST(w.variant_id AS CHAR) LIKE :q_variant_id
            ...
            OR DATE_FORMAT(w.created_at, '%Y-%m-%d %H:%i:%s') LIKE :q_created_at
            OR DATE_FORMAT(w.updated_at, '%Y-%m-%d %H:%i:%s') LIKE :q_updated_at
        )";

        $searchValue = '%' . $q . '%';

        $params[':q_wishlist_id'] = $searchValue;
    ...
    $sql = "
        ...
        ORDER BY $sortColumn $sortOrder, w.id DESC
